...
class DE10Nano_BRAM:
    def __init__(self):
        self.WEIGHTS = np.zeros(2 * 1024 * 1024, dtype=np.int32) 
        self.BIAS    = np.zeros(128 * 1024, dtype=np.int32)
        self.BUFFER_A = np.zeros(2048 * 1024, dtype=np.uint8) 
        self.BUFFER_B = np.zeros(2048 * 1024, dtype=np.uint8)

# ──────────────────────────────────────────────────────────────
# 2. INSTRUCCIÓN INT8 (Ahora con offsets de memoria)
# ──────────────────────────────────────────────────────────────
class ConvLayerDescINT8:
    def __init__(self, name, input_name, output_name, in_c, out_c, h, w, k_size, stride, pad, 
        ...
        # Punteros para la FPGA
        self.buf_in = 0  
        self.buf_out = 0 
        self.in_offset = 0   # Dirección de lectura
        self.out_offset = 0  # Dirección de escritura

        self.out_h = (h + 2 * pad - k_size) // stride + 1
        self.out_w = (w + 2 * pad - k_size) // stride + 1

# ──────────────────────────────────────────────────────────────
# 3. ASIGNADOR DE MEMORIA (El Linker Inteligente)
# ──────────────────────────────────────────────────────────────
def assign_hardware_buffers(schedule, initial_input_name):
    # Diccionario: nombre_tensor -> (id_buffer, offset)
    buffer_map = {initial_input_name: (0, 0)} 
    
    # Punteros de memoria libre (como un Arena Allocator)
    free_offset = {0: 0, 1: 0}
    
    for layer in schedule:
        # Recuperamos dónde está la entrada
        buf_in_id, in_offset = buffer_map[layer.input_name]
        layer.buf_in = buf_in_id
        layer.in_offset = in_offset
        
        if layer.save_to_cpu:
            layer.buf_out = 2 
            layer.out_offset = 0
        else:
            layer.buf_out = 1 - layer.buf_in 
            
            # Buscamos hueco libre en el buffer de destino
            tamaño_salida = layer.out_c * layer.out_h * layer.out_w
            layer.out_offset = free_offset[layer.buf_out]
            
            # Registramos dónde va a estar este tensor para las siguientes capas
            buffer_map[layer.output_name] = (layer.buf_out, layer.out_offset)
            
            # Avanzamos el puntero para que las ramas paralelas no lo pisen
            free_offset[layer.buf_out] += tamaño_salida

# ──────────────────────────────────────────────────────────────
# 4. EL MOTOR MATEMÁTICO INT8
# ──────────────────────────────────────────────────────────────
                    ...
                    for kh in range(layer.k_size):
                        for kw in range(layer.k_size):
                            ...
                            if 0 <= ih < layer.h and 0 <= iw < layer.w:
                                ...
                                val_w = bram.WEIGHTS[w_idx] - layer.w_zp
                                acc += (val_x * val_w)
                
                y_float = acc * layer.M
                y_int = int(round(y_float)) + layer.y_zp
                
                y_clamped = max(0, min(255, y_int))
                ptr_out[out_idx] = np.uint8(y_clamped)

# ──────────────────────────────────────────────────────────────
# 5. ORQUESTADOR PING-PONG SEGURO
# ──────────────────────────────────────────────────────────────
def run_network_pingpong_int8(bram, schedule, input_image_1D):
    # Imagen al inicio del Buffer A
    np.copyto(bram.BUFFER_A[:len(input_image_1D)], input_image_1D.astype(np.uint8))
    
    buffers = {0: bram.BUFFER_A, 1: bram.BUFFER_B}
    outputs_para_cpu = {}
    
    for i, layer in enumerate(schedule):
        # Le enviamos al motor matemático solo la "ventana" de memoria que le toca leer
        ptr_in = buffers[layer.buf_in][layer.in_offset:]
        ...
        if layer.buf_out == 2:
            ...
            elementos_salida = layer.out_c * layer.out_h * layer.out_w
            # Le enviamos la "ventana" exacta donde tiene que escribir
            ptr_out = buffers[layer.buf_out][layer.out_offset : layer.out_offset + elementos_salida]
...
def parse_onnx_qlinear(onnx_path, fpga_bram):
    ...
    for node in graph.node:
        if node.op_type == 'QLinearConv':
            ...
            w_len = len(w_flat)
            fpga_bram.WEIGHTS[current_w_offset : current_w_offset + w_len] = w_flat.astype(np.int32)
            ...
            b_len = len(b_flat)
            fpga_bram.BIAS[current_b_offset : current_b_offset + b_len] = b_flat.astype(np.int32)
...
if __name__ == "__main__":
    ...
    assign_hardware_buffers(mi_schedule, initial_input_name)
    ...
    print("\n🚀 INICIANDO MOTOR FPGA INT8 PING-PONG (CON OFFSETS)...\n")
    resultados_finales = run_network_pingpong_int8(fpga_mem, mi_schedule, tensor_entrada)
