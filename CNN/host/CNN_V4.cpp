...
std::vector<std::array<float, 4>> generate_priors(int input_width = 320, int input_height = 240) {
    ...
    // ¡Ojo! Hemos eliminado el {30,40} del principio
    std::vector<std::pair<int,int>> feature_maps = {{15,20},{8,10},{4,5}};
    std::vector<std::vector<int>> min_sizes = {
        {32,48}, {64,96}, {128,192,256}
    };
    std::vector<int> steps = {16,32,64};
    ...
    return prior_boxes; // Ahora esto devolverá exactamente 820 cajas
}
...
int main(int argc, char* argv[]) {
    ...
    */
    ...
    const char* output_names[] = {"289","329","361","303","343","373"};
    ...
    while (true) {
        ...
        // Obtenemos los punteros de los 6 tensores
        float* scores_15x20 = output_tensors[0].GetTensorMutableData<float>();
        float* scores_8x10  = output_tensors[1].GetTensorMutableData<float>();
        float* scores_4x5   = output_tensors[2].GetTensorMutableData<float>();

        float* boxes_15x20  = output_tensors[3].GetTensorMutableData<float>();
        float* boxes_8x10   = output_tensors[4].GetTensorMutableData<float>();
        float* boxes_4x5    = output_tensors[5].GetTensorMutableData<float>();

        size_t num_boxes = priors.size(); // Ahora es 820
        std::vector<std::array<float,2>> conf(num_boxes);
        std::vector<std::array<float,4>> loc(num_boxes);

        size_t offset = 0;

        // Función rápida (lambda) para copiar los datos de cada rama a nuestro vector final
       // Función rápida (lambda) para copiar los datos y aplicar SOFTMAX
        auto copy_data = [&](float* src_scores, float* src_boxes, int count) {
            for(int i=0; i<count; ++i) {
                // 1. Leer los logits crudos
                float logit_background = src_scores[i*2+0];
                float logit_face       = src_scores[i*2+1];
                
                // 2. Aplicar Softmax para obtener la probabilidad real (0.0 a 1.0)
                // Restamos el máximo por estabilidad numérica (evita infinito si el número es muy grande)
                float max_logit = std::max(logit_background, logit_face);
                float exp_bg   = std::exp(logit_background - max_logit);
                float exp_face = std::exp(logit_face - max_logit);
                float sum_exp  = exp_bg + exp_face;
                
                conf[offset+i][0] = exp_bg / sum_exp;   // Probabilidad de NO ser cara
                conf[offset+i][1] = exp_face / sum_exp; // Probabilidad de SÍ ser cara

                // Las cajas (boxes) no necesitan softmax, se copian igual
                loc[offset+i][0]  = src_boxes[i*4+0];
                loc[offset+i][1]  = src_boxes[i*4+1];
                loc[offset+i][2]  = src_boxes[i*4+2];
                loc[offset+i][3]  = src_boxes[i*4+3];
            }
            offset += count;
        };

        // Copiamos las 600 cajas de la primera rama, 160 de la segunda y 60 de la tercera
        copy_data(scores_15x20, boxes_15x20, 600);
        copy_data(scores_8x10, boxes_8x10, 160);
        copy_data(scores_4x5, boxes_4x5, 60);
