...
int main(int argc, char const *argv[])
{
    ...
    cl_command_queue_properties props = CL_QUEUE_OUT_OF_ORDER_EXEC_MODE_ENABLE | CL_QUEUE_PROFILING_ENABLE; // Habilitar profiling para medir tiempos
    ...
    while (true)
    {
        ...
        // 2. Soltar control de 'next' (Unmap asíncrono)
        ...
        // 3. Ejecutar Kernel 'next' (Esperando a que los Unmaps terminen)
        ...
        // Solicitar recuperar el control de 'next' (Map asíncrono)
        ...
        //4. EL ÚNICO BLOQUEO: Esperar a que la CPU recupere el control de 'current'
        ...
        */
        ...
        // 6. Mostrar el resultado (Cero copias, imshow lee la memoria compartida)
        cv::imshow("Sobel Compartido FPGA", mat_out[current]);

        // 7. Liberar eventos para no llenar la memoria RAM
        ...
        // 8. Ping-Pong: Intercambiar índices
        current = 1 - current;
        next = 1 - next;
